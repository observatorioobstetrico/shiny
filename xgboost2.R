...
saveRDS(srag_train,"dados_treino.rds")
...
saveRDS(xgb_res,"tunning_smotenc_ML.rds")
...
model <- final_xgb %>% fit(srag_data)

saveRDS(model,"modelo_xgb.rds")

model <- readRDS("modelo_xgb.rds")

dados_nespecificado <- readRDS("dados_nespecificado.rds")

data_teste <- predict(model,dados_nespecificado)

data_teste$classi_fin_pred <- preds$.pred_class
